...
async function genSlide(ai, opts) {
  const { prompt, outFile, imageSize, aspectRatio, label, images } = opts;
 ...
 *
 * @param {Array} slidesData - Array of { path, texts? }
 ...
 *   images — optional array of image overlays:
 *     { path, x, y, w, h, rounding? }
 */
async function buildPptx(slidesData, outFile) {
  ...
  for (const sd of slidesData) {
    ...
    const imgOverlays = typeof sd === "object" && sd !== null ? sd.images : undefined;
    ...
    // Image overlays (photos, logos, etc.)
    if (imgOverlays && imgOverlays.length > 0) {
      for (const img of imgOverlays) {
        if (img.path && fs.existsSync(img.path)) {
          const ext = path.extname(img.path).slice(1).toLowerCase();
          const mime = ext === "jpg" ? "jpeg" : ext;
          const b64 = `data:image/${mime};base64,` + fs.readFileSync(img.path, "base64");
          const imgOpts = { data: b64, x: img.x, y: img.y, w: img.w, h: img.h };
          if (img.rounding) imgOpts.rounding = true;
...
async function run(config) {
  ...
  async function worker() {
    while (queue.length > 0) {
      ...
      // Auto-append no-text instruction when native text overlays are defined
      if (s.texts) {
  ...
  // Build slide data with optional text/image overlays
  const slidesData = paths.map((p, i) => {
    const s = slides[i];
    if (s.texts || s.overlayImages) {
      return { path: p, texts: s.texts, images: s.overlayImages };
    }
    return p; // legacy: plain path string
  });
 ...
 * @param {string}   [opts.videoModel="veo-3.1-generate-preview"]
 */
...
/**
 * Batch pipeline: submit slides as Gemini batch jobs (chunked), poll, build PPTX.
 * ~50% cheaper than `run`; turnaround up to 24h (usually much faster for images).
 * Chunks to work around Node V8 string limit (~500MB) when decoding inlined responses.
 *
 * @param {Object}   config
 *   slideDir, outFile, slides, getStyle, imageSize?, model?
 * @param {number}   [config.pollIntervalMs=15000]
 * @param {number}   [config.chunkSize=10]  — slides per batch job (10 keeps response <50MB for 2K images)
 */
async function runBatch(config) {
  const {
    slideDir, outFile, slides, getStyle,
    imageSize, model, pollIntervalMs = 15000, chunkSize = 10,
  } = config;
  ...
  const total = slides.length;
  const cached = new Array(total).fill(null);
  const toSubmit = [];
  for (let i = 0; i < total; i++) {
    const padded = String(i + 1).padStart(2, "0");
    const p = path.join(slideDir, `slide-${padded}.png`);
    if (fs.existsSync(p) && fs.statSync(p).size > 10000) cached[i] = p;
    else toSubmit.push({ idx: i, slide: slides[i] });
  }
  console.log(`${cached.filter(Boolean).length} cached, ${toSubmit.length} to submit as batch (chunk=${chunkSize}).`);
  const paths = [...cached];

  function buildRequest({ idx, slide: s }) {
    const prefix = getStyle(s.style || "normal");
    let fullPrompt = prefix + "\n\n" + s.prompt;
    if (s.texts) {
      fullPrompt += "\n\nCRITICAL: DO NOT render any text, words, labels, numbers, or letters anywhere on the image.";
    }
    const parts = [];
    ...
    const reqConfig = { responseModalities: ["IMAGE", "TEXT"] };
    if (imageSize) reqConfig.imageConfig = { aspectRatio: "16:9", imageSize };
    return {
      contents: [{ role: "user", parts }],
      config: reqConfig,
      metadata: { key: `slide-${String(idx + 1).padStart(2, "0")}` },
    };
  }
  ...
  for (let i = 0; i < toSubmit.length; i += chunkSize) {
    chunks.push(toSubmit.slice(i, i + chunkSize));
  }
  console.log(`Split into ${chunks.length} batch job(s).`);

  async function runChunk(chunk, chunkIdx) {
    const requests = chunk.map(buildRequest);
    console.log(`  [chunk ${chunkIdx + 1}/${chunks.length}] submitting ${requests.length} slides...`);
    const job = await ai.batches.create({ model: modelName, src: requests });
    console.log(`  [chunk ${chunkIdx + 1}] job=${job.name}`);

    let elapsed = 0;
    while (true) {
      await new Promise(r => setTimeout(r, pollIntervalMs));
      elapsed += pollIntervalMs;
      const updated = await ai.batches.get({ name: job.name });
      const mins = (elapsed / 60000).toFixed(1);
      console.log(`  [chunk ${chunkIdx + 1}][${mins}m] state=${updated.state}`);
      if (["JOB_STATE_SUCCEEDED", "JOB_STATE_FAILED", "JOB_STATE_CANCELLED", "JOB_STATE_EXPIRED"].includes(updated.state)) {
        if (updated.state !== "JOB_STATE_SUCCEEDED") {
          console.error(`  [chunk ${chunkIdx + 1}] ${updated.state}:`, JSON.stringify(updated.error || {}).slice(0, 500));
          return;
        }
        const responses = updated.dest?.inlinedResponses || [];
        for (const r of responses) {
          const match = r.metadata?.key?.match(/slide-(\d+)/);
          if (!match) continue;
          const idx = parseInt(match[1]) - 1;
          const resParts = r.response?.candidates?.[0]?.content?.parts || [];
          const imgPart = resParts.find(p => p.inlineData);
          const padded = String(idx + 1).padStart(2, "0");
          const outImg = path.join(slideDir, `slide-${padded}.png`);
          if (imgPart) {
            const buf = Buffer.from(imgPart.inlineData.data, "base64");
            fs.writeFileSync(outImg, buf);
            paths[idx] = outImg;
            console.log(`    slide-${padded}: ${(buf.length / 1024).toFixed(0)}KB`);
          } else {
            console.log(`    slide-${padded}: NO IMAGE · ${r.error?.message || "empty"}`);
          }
        }
        return;
      }
  ...
  const slidesData = paths.map((p, i) => {
    const s = slides[i];
    if (s.texts || s.overlayImages) return { path: p, texts: s.texts, images: s.overlayImages };
    return p;
  });
  await buildPptx(slidesData, outFile);
  console.log(`\nDone: ${outFile} (${paths.filter(Boolean).length}/${total} slides)`);
...
module.exports = { genSlide, buildPptx, run, runBatch, runCards, animateCard, runVideoCards, SW, SH };
